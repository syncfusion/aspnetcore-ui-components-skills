# MySQL Database Binding in ASP.NET Core Pivot Table
...
## Table of Contents
- [Creating a Web API Service to Fetch MySQL Data](#creating-a-web-api-service-to-fetch-mysql-data)
- [Connecting the Pivot Table to a MySQL Database](#connecting-the-pivot-table-to-a-mysql-database-using-the-web-api-service)
...
In the **PivotController.cs** file, define a **Get** method that calls **GetMySQLResult** to retrieve data from the MySQL database as a **DataTable**. Then, use **JsonConvert.SerializeObject** from the **Newtonsoft.Json** library to convert the **DataTable** into JSON format. This JSON data will be used by the Pivot Table component.
...
2. The browser will display the JSON data, confirming successful MySQL connection and data retrieval.
...
- **MySqlConnection**: Establishes connection to MySQL using connection string with server, database, user ID, and password parameters.
- **MySqlCommand**: Executes SQL queries against the MySQL database to fetch the required data.
- **MySqlDataAdapter**: Populates DataTable with query results, which is then serialized to JSON for Pivot Table consumption.
...
Explore a complete example of the ASP.NET Core Pivot Table integrated with a MySQL database in this [GitHub repository](https://github.com/SyncfusionExamples/how-to-bind-MySQL-database-to-pivot-table).
