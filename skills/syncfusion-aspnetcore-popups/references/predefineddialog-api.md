# Predefined Dialogs — API Reference (ASP.NET Core)

> **Source:** [Syncfusion EJ2 Predefined Dialogs documentation](https://help.syncfusion.com/cr/aspnetcore-js2/syncfusion.ej2.popups.dialog.html)
>
...
In your Razor view, access via the global `ej.popups` namespace (no explicit import needed after including `ej2.min.js`):

```javascript
ej.popups.DialogUtility.alert(options);
ej.popups.DialogUtility.confirm(options);
```
...
| `click` | `function` | Callback when button is clicked |
...
|---|---|---|
| `hide` | `hide(event?: Event)` | Programmatically closes the dialog |
